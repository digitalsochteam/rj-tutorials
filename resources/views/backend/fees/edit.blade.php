...
@section('page-title', 'Edit Fee Entry')

@section('content')

    <div style="margin-bottom:1.75rem;display:flex;align-items:center;justify-content:space-between;">
        <div>
            <h2 style="font-size:1.3rem;font-weight:800;letter-spacing:-0.025em;">Edit Fee Entry</h2>
            <p style="font-size:0.875rem;color:#64748b;margin-top:0.2rem;">Update "{{ $fee->type }}" under
                "{{ $fee->course_name }}".</p>
        </div>
    ...
    <form method="POST" action="{{ route('fees.update', $fee) }}">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="course_name" name="course_name"
                            value="{{ old('course_name', $fee->course_name) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="type" name="type" value="{{ old('type', $fee->type) }}" required>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="form-body">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="number" id="fees" name="fees" value="{{ old('fees', $fee->fees) }}" min="0"
                            step="0.01">
                    </div>
                    <div class="form-group">
                        <label for="discount">Discount (₹)</label>
                        <input type="number" id="discount" name="discount" value="{{ old('discount', $fee->discount) }}"
                            min="0" step="0.01">
                    </div>
                    <div class="form-group">
                        <label for="after_discount">After Discount (₹)</label>
                        <input type="number" id="after_discount" name="after_discount"
                            value="{{ old('after_discount', $fee->after_discount) }}" min="0" step="0.01">
                    </div>
                    <div class="form-group">
                        <label for="sort_order">Sort Order</label>
                        <input type="number" id="sort_order" name="sort_order"
                            value="{{ old('sort_order', $fee->sort_order) }}" min="0">
                    </div>
                    <div class="form-group">
                        <label>Status</label>
                        <label
                            ...
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $fee->is_active) ? 'checked' : '' }}>
                            Show on website
                        </label>
                    </div>
                </div>
        ...
        <div style="display:flex;justify-content:flex-end;gap:.75rem;">
            ...
            <button type="submit" class="btn-primary">Update Fee Entry</button>
        </div>
