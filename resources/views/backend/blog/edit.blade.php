    ...
    <form method="POST" action="{{ route('blog.update', $blog) }}" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="form-body">
                <div class="form-grid">
                    <div class="form-group full">
                        <label>Meta Title <small style="color:#94a3b8;">(leave blank to use post title &mdash; 50-60 chars)</small></label>
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $blog->meta_title) }}"
                            placeholder="e.g. Science Exam Tips | RJ Tutorials" maxlength="80"
                            oninput="countChars(this,'meta_title_count',60)">
                        <span id="meta_title_count" style="font-size:.75rem;color:#94a3b8;">{{ strlen(old('meta_title', $blog->meta_title ?? '')) }} / 60</span>
                    </div>
                    <div class="form-group full">
                        <label>Meta Description <small style="color:#94a3b8;">(leave blank to use excerpt &mdash; 150-160 chars)</small></label>
                        <textarea name="meta_description" rows="3" maxlength="165"
                            placeholder="Brief description shown in Google search results..."
                            oninput="countChars(this,'meta_desc_count',160)">{{ old('meta_description', $blog->meta_description) }}</textarea>
                        <span id="meta_desc_count" style="font-size:.75rem;color:#94a3b8;">{{ strlen(old('meta_description', $blog->meta_description ?? '')) }} / 160</span>
                    </div>
                    <div class="form-group full">
